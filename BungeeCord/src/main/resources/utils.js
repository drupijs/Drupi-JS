 ...
 * Modified by Drupi author, Hundeklemmen.
 *
 */
...
(function () {
  ...
  function Require (id, parent) {
    var core;
    var native_;
    var fileSplit = java.io.File.separator;
    var file = Require.resolve("plugins" + fileSplit + "Drupi" + fileSplit + "scripts" + fileSplit + "modules" + fileSplit + "" + id, parent);
    if(id.startsWith("./")){
      file = Require.resolve("plugins" + fileSplit + "Drupi" + fileSplit + "scripts" + fileSplit + "" + id, parent);
    }
    if (!file && parent &&parent+"".startsWith("plugins" + fileSplit + "Drupi" + fileSplit + "scripts" + fileSplit + "modules")) {
      file = Require.resolve("." + fileSplit + ".." + fileSplit + "" + id, parent);
    }
    if (!file && parent &&parent+"".startsWith("plugins" + fileSplit + "Drupi" + fileSplit + "scripts" + fileSplit + "modules")) {
      file = Require.resolve("." + fileSplit + ".." + fileSplit + "" + id.toLowerCase(), parent);
    }
    if (!file) {
      file = Require.resolve(id, parent);
    }
  ...
  function loadJSON (file) {
    var json = JSON.parse(readFile(file), false);
  ...
  function resolveAsDirectory (id, root) {
    ...
    if (file.exists()) {
      try {
        var body = readFile(file.getCanonicalPath(), false);
        var package_ = JSON.parse(body);
        if(package_.name){
          drupihelper.moduleLoaded(package_.name, package_.version || "1.0.0")
        }
        if (package_.main) {
    ...
    drupihelper.moduleLoaded(base, "1.0.0")
    return resolveAsFile('index.js', base);
  }
